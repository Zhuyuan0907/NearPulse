...
| OSM 場域圖資（離線快照） | ✅ | 本機解析 .osm.pbf（**零限流**，全台 101 秒）→ 778 場域 / 1340 出口 / 455KB |
| 方向地標（最近有名街道） | ✅ | 覆蓋率 23% → **96%**，全部本機計算 |
| 無障礙資料（OSM wheelchair） | ✅ | 42% 出口有標註；100 個場域可做無障礙疏散判斷 |
...
| 端到端 API 測試腳本 | ✅ | `server/test/e2e.sh`——48 項檢查，不需任何 API 金鑰 |
...
- AI 不在關鍵路徑：✅ 無金鑰環境下 e2e 48 項全通過（降級路徑＝主路徑）
