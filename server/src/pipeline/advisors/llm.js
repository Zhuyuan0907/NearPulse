...
const ADVICE_TEMPLATES = {
  ...
  /**
   * 攻擊的建議與其他類型**本質不同**：其他事故是「往出口移動」，
   * 攻擊是「遠離、隱蔽、不要製造聲響與注意」。把攻擊套用一般疏散文案
   * 會把人往危險推——這也是為什麼它必須是獨立的事件類型而非「其他」。
   *
   * 用詞參考各國反恐應變的通則（Run / Hide / Tell）：
   * 能離開就立刻離開；不能離開就找掩蔽並保持安靜；然後才是通報。
   */
  attack: {
    active: '立刻遠離現場，不要圍觀或拍攝。往反方向離開；無法離開時請找掩蔽物、關靜音並保持安靜，待安全後再報警。',
    candidate: '未經確認：附近有人回報疑似持械攻擊。請提高警覺並遠離該區域，若您在現場請協助確認。',
  },
  medical: {
